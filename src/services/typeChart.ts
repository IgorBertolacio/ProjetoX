...
export function getEffectiveness(attackerType: string, defenderType: string): number {
    const atk = attackerType.toLowerCase() as PokemonType;
    const def = defenderType.toLowerCase() as PokemonType;

    if (!TYPE_CHART[atk] || !TYPE_CHART[atk][def]) return 1;
    return TYPE_CHART[atk][def];
}

export function getDualEffectiveness(attackerType: string, defenderTypes: string[]): number {
    let multiplier = 1;
    for (const defType of defenderTypes) {
        multiplier *= getEffectiveness(attackerType, defType);
    }
    return multiplier;
}
